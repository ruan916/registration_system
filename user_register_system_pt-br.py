...
class admin_window:
    ...
    def search_record(self):
        try:
            ...
            if(length==0):
                messagebox.showinfo("ERRO","SEM RESULTADOS")
            if(length!='0'):
                i=0
            ...
            raise messagebox.showerror("ERRO","NÃO FOI POSSÍVEL ACHAR ESSES DADOS")

    def reset_db(self):
        yesno=messagebox.askquestion("REDEFINIR BANCO DE DADOS","TODO O BANCO DE DADOS SERÁ PERDIDO, DESEJA CONTINUAR?")
        if(yesno=='yes'):
            self.theCursor.execute("DROP TABLE users")
            messagebox.showinfo("BANCO DE DADOS REDEFINIDO","BANCO DE DADOS REDEFINIDO")
            self.setup_db()
            self.update_tree()
    ...
    def write_record(self):
        if(self.username_vl.get()!="" and self.password_vl.get()!=""):
            try:
                ...
                messagebox.showerror("ERRO","ESSE USUÁRIO JÁ SE ENCONTRA NO BANCO DE DADOS")
            except:
                messagebox.showerror("ERRO","FALHA NA GRAVAÇÃO DOS DADOS")
            finally:
                self.update_tree()
        else:
    ...
    def __init__(self):
        ...
        self.admin_window.title("CONTROLE DO BANCO DE DADOS DO ADM")

        self.username_lbl = Label(self.admin_window,text="USUÁRIO: ",font="Helvetica, 16",bg='light green',fg='#001a00')
        ...
        self.password_lbl = Label(self.admin_window,text="SENHA: ",font="Helvetica, 16",bg='light green',fg='#001a00')
        ...
        self.bt_adm_register = Button(self.admin_window,text="REGISTRAR-SE",font="Helvetica, 16",command=self.write_record)
        self.bt_adm_register.configure(bg='#001a00',fg='white')
        self.bt_adm_register.grid(row=0,column=3,padx=2,pady=2,sticky=W+E)

        self.bt_adm_delete = Button(self.admin_window,text="DELETAR",font="Helvetica, 16",command=self.delete_record)
        ...
        Label(self.admin_window,text="PESQUISAR PELO NOME: ",font="Helvetica, 16",bg='light green',fg='#001a00').grid(row=5,column=0,columnspan=2,pady=5,sticky=E)
        ...
        self.bt_adm_search = Button(self.admin_window,text="PESQUISAR",font="Helvetica, 16",command=self.search_record)
        self.bt_adm_search.configure(bg='#001a00',fg='white')
        self.bt_adm_search.grid(row=5,column=3,padx=5,pady=5,sticky=W+E)

        self.bt_adm_update_database = Button(self.admin_window,text="ATUALIZAR",font="Helvetica, 16",command=self.update_database)
        ...
        self.bt_adm_reset = Button(self.admin_window,text="REDEFINIR BANCO DE DADOS",font="Helvetica, 16",command=self.reset_db)
...
class register_window:
    ...
    def new_user(self):
        try:
            ...
            messagebox.showerror("ERRO","NÃO FOI POSSÍVEL REGISTRAR ESSES DADOS")
        finally:
            self.sqlite_var.commit()
            self.theCursor.execute("SELECT * FROM users")
    ...
    def __init__(self):
        ...
        self.register_window.title("REGISTRAR USUÁRIO")
        self.register_window.resizable(False,False)
        
        self.password_text = StringVar()
        self.username_text = StringVar()

        Label(self.register_window,text="REGISTRAR-SE",font="Helvetica, 20",bg='light green',fg='#001a00').grid(row=0,column=0,columnspan=3,padx=10,pady=10,sticky=W+E)
        Label(self.register_window,text="USUÁRIO: ",font="Helvetica, 16",bg='light green',fg='#001a00').grid(row=1,column=0,sticky=E)
        Label(self.register_window,text="SENHA: ",font="Helvetica, 16",bg='light green',fg='#001a00').grid(row=2,column=0,sticky=E)
        ...
        bt_add_user = Button(self.register_window,font="Helvetica, 16",text="REGISTRAR-SE",command=self.new_user)
        bt_add_user.configure(bg='#001a00',fg='white')
        bt_add_user.grid(row=1,column=2,rowspan=2,padx=10)

        Label(self.register_window,text="",bg='light green').grid(row=3,column=1,sticky=W+E)
...
class login_window:
    ...
    def log(self):
        try:
            ...
            if(self.var.get()==2 and self.username_text.get()=="admin" and self.password_text.get()=="aps"):
                ...
                flag = 1
            if(flag==0):
                messagebox.showinfo("ERRO","USUÁRIO OU SENHA INCORRETA")
        except:
            messagebox.showerror("ERRO","NÃO FOI POSSÍVEL FAZER LOGIN")
        finally:
            self.password_text.set("")
            self.username_text.set("")
    ...
    def __init__(self):
        ...
        Label(self.login_window,text="USUÁRIO: ",font="Helvetica, 16",bg='light green',fg='#001a00').grid(row=1,column=0,sticky=E)
        Label(self.login_window,text="SENHA: ",font="Helvetica, 16",bg='light green',fg='#001a00').grid(row=2,column=0,sticky=E)
        ...
        Radiobutton(self.login_window,text="USUÁRIO",bg='light green',fg='#001a00',variable=self.var,value=1).grid(row=3,column=0,pady=5)
        Radiobutton(self.login_window,text="ADM (SOMENTE PARA AUTORIZADOS)",bg='light green',fg='#001a00',variable=self.var,value=2).grid(row=3,column=1)
...
class main_window:
    def login(self):
        try:
            ...
            raise Exception("A tela de login não está funcionando")

    def register(self):
        try:
            register_window()
        except:
            raise Exception("A tela de registro não está funcionando")

    def main_quit(self):
        if messagebox.askokcancel("Você realmente deseja sair?", "Você realmente deseja sair?"):
            self.root.destroy()
    ...
    def __init__(self):
        ...
        self.bt_main_register = Button(self.root,text="REGISTRAR-SE",font="Helvetica, 16",command=self.register)
        ...
        self.bt_main_quit = Button(self.root,text="FECHAR",font="Helvetica, 16",command=self.main_quit)
        ...
        Label(self.root,text="",bg='light green').grid(row=4,column=0,sticky=W+E)
